...
class RelayServer:
    ...
    def handle_tcp_message(self, data, client_id):
        ...
        elif msg_type == 'STRESS_TEST_START':
            # Stress test functionality - development only
            bot_count = data.get('bot_count', 10)
            print(f"Starting stress test with {bot_count} bots requested by {client_id}")
            
            # Create virtual bot clients (they only exist in memory)
            bot_ids = []
            for i in range(bot_count):
                bot_client_id = f"bot_{self.next_client_id}"
                self.next_client_id += 1
                
                # Register bot in clients dictionary with minimal data
                with self.clients_lock:
                    self.clients[bot_client_id] = {
                        'tcp_socket': None,  # No real socket
                        'public_ip': '127.0.0.1',
                        'public_port': 0,
                        'last_heartbeat': time.time(),
                        'is_bot': True  # Mark as a bot
                    }
                
                # Set a name for the bot
                self.player_names[bot_client_id] = f"StressBot{i+1}"
                
                # Add bot to the same room as the requester
                with self.rooms_lock:
                    for room_id, room_data in self.game_rooms.items():
                        if client_id in room_data['players']:
                            # Add bot to this room
                            room_data['players'].append(bot_client_id)
                            break
                
                bot_ids.append(bot_client_id)
            
            # Send confirmation with bot IDs
            with self.clients_lock:
                if client_id in self.clients:
                    self.send_tcp_message(self.clients[client_id]['tcp_socket'], {
                        'type': 'STRESS_TEST_BOTS_CREATED',
                        'bot_ids': bot_ids
                    })
            
            print(f"Created {len(bot_ids)} bots for stress test")

        elif msg_type == 'STRESS_TEST_STOP':
            # Clean up all bots
            with self.clients_lock:
                bot_ids = [cid for cid, data in self.clients.items() if data.get('is_bot', False)]
                
                for bot_id in bot_ids:
                    # Remove bot from rooms
                    with self.rooms_lock:
                        for room_id, room_data in self.game_rooms.items():
                            if bot_id in room_data['players']:
                                room_data['players'].remove(bot_id)
                    
                    # Remove from clients
                    del self.clients[bot_id]
                    
                    # Remove from player names/positions
                    if bot_id in self.player_names:
                        del self.player_names[bot_id]
                    if bot_id in self.player_positions:
